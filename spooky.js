	...
	fs.readFile("./password.txt", function(err, data) {
		...
		client.on("data", function(msg) {
			...
			for(var i = 0; i < msg.channelManager.channels.length; i++) {
				...
				if(1) { //if(channel._id === SPOOP_CHANNEL) {
					...
					for(var j in users) {
                        client.sendArray([{m: "admin message", password: password,
                            msg: {"m": "name", "_id": users[j]._id, "name": spoop_text(users[j].name)}}]);
                        client.sendArray([{m: "admin message", password: password,
                            msg: {"m": "color", "_id": users[j]._id, "color": "#000000"}}]);
			client.sendArray([{m: "admin message", password: password,
                            msg: {"m": "user_flag", "_id": users[j]._id, "key": "chat_curse_1","value":1}}]);
			client.sendArray([{m: "admin message", password: password,
                            msg: {"m": "user_flag", "_id": users[j]._id, "key": "chat_curse_2","value":1}}]);
			client.sendArray([{m: "admin message", password: password,
                            msg: {"m": "user_flag", "_id": users[j]._id, "key": "freeze_name","value":1}}]);
			/*client.sendArray([{m: "admin message", password: password,
                    ...
                    setTimeout(function() {
                        for(var j in users) {
                            client.sendArray([{m: "admin message", password: password,
                                msg: {"m": "name", "_id": users[j]._id, "name": users[j].name}}]);
                            client.sendArray([{m: "admin message", password: password,
                                msg: {"m": "color", "_id": users[j]._id, "color": users[j].color}}]);
			    client.sendArray([{m: "admin message", password: password,
                                msg: {"m": "user_flag", "_id": users[j]._id, "key": "chat_curse_1","value":0}}]);
			    client.sendArray([{m: "admin message", password: password,
                                msg: {"m": "user_flag", "_id": users[j]._id, "key": "chat_curse_2","value":0}}]);
			    client.sendArray([{m: "admin message", password: password,
                                msg: {"m": "user_flag", "_id": users[j]._id, "key": "freeze_name","value":0}}]);
                        }
                        setTimeout(function() {
                            process.exit();
                        }, 1000);
