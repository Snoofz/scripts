...
$(function() {
    ...
    var sendCursorUpdateDelay = 50; // The time delay for you to send a cursor update (Default: 50 ms).
    var defaultBGcolor = "#3b5054";
    //var backgroundImage = "http://puu.sh/vwYCc/b8df445b2e.png";
    var backgroundImage = "http://cdn.wallpapersafari.com/83/59/pv8EL0.jpg"; //set to "" for colors.
    var bottomBarColor = "#000";
    var gPianoMutes = []; //Add _ids here to Mute someone forever!
    var gChatMutes = []; //Add _ids here to Mute someone forever!
    var DEFAULT_VELOCITY = 1;

    //FAK HAITCHE
    var twitchEmotes = "true";//Set to false for no emotes
    var emotes = ["FeelsBadMan", "FeelsGoodMan", "LUL", "KappaPride", "Kappa",  "Kreygasm", "DansGame", "4Head", "EleGiggle", "BabyRage", "OpieOP", "ResidentSleeper", "NotLikeThis", "FailFish", "PogChamp", "VoHiYo", "SMOrc", "SeemsGood"];
    var sources = [
        "https://puu.sh/u8xfc/ec2f89e873.png",//FeelsBadMan
        "https://puu.sh/u8xfl/b3c10a805a.png",//FeelsGoodMan
        "https://puu.sh/u8xfJ/a9c644b6e4.png",//LUL
        "https://puu.sh/u8xfw/06457d6140.png",//KappaPride
        "https://puu.sh/u8xfr/c33f45e429.png",//Kappa
        "https://puu.sh/u8xfC/4978f4541e.png",//Kreygasm
        "https://puu.sh/u8xeM/81c06c6d68.png",//DansGame
        "http://puu.sh/u8xrm/dd3798b846.png",//4Head
        "https://puu.sh/u8xeY/7c41e1c960.png",//EleGiggle
        "https://puu.sh/u8xeB/faf7209ee4.png",//BabyRage
        "https://puu.sh/u8xfY/a71d64af08.png",//OpieOP
        "https://puu.sh/u8xg7/59e34c028d.png",//ResidentSleeper
        "https://puu.sh/u8xfS/da1412c2ed.png",//NotLikeThis
        "https://puu.sh/u8xf5/114f15b507.png",//FailFish
        "https://puu.sh/u8xg1/83b9a0f37c.png",//PogChamp
        "https://puu.sh/u8xgJ/e248847664.png",//VoHiYo
        "https://puu.sh/u8xgx/a67cddad86.png",//SMOrc
        "https://puu.sh/u8xgi/c9601a63c5.png"//SeemsGood
    ];
    ///////////////////////////////////
    ...
    (function() {
        var originalParticipantAdded = function(part) {
        ...
        var originalParticipantRemoved = function(part) {
            // remove nameDiv
            var nd = $(part.nameDiv);
        ...
        var originalParticipantUpdate =  function(part) {
            var name = part.name || "";
        ...
        //Add the listeners to the client
        gClient.on("participant added", originalParticipantAdded);
        gClient.on("participant removed", originalParticipantRemoved);
        gClient.on("participant update", originalParticipantUpdate);
        gClient.on("ch", originalCH);
    })();
    ...
    // Background color
    (function() {
        var old_color1 = new Color(defaultBGcolor);
        var old_color2 = new Color(defaultBGcolor);
        ...
        if (backgroundImage === "") {
            ...
            document.getElementById("bottom").style.background = bottomBarColor;
        }
    })();
        ...
        var participantMenu = function(part) {
            ...
            if(gClient.isOwner()) {
                $('<div class="menu-item give-crown">Give Crown</div>').appendTo(menu)
                    .on("mousedown touchstart", function(evt) {
                    var response = prompt("Please type 'yes' to pass the crown to this person.");
                    if(response === "yes") {
...
<a href="http://twitter.com/home?status='+encodeURIComponent(location.href)+'" target="_blank">Tweet</a>', duration: 25000});
    ...
    var chat = (function() {
        ...
        gClient.on("a", function(msg) {
            if(msg.a.startsWith("Banned ") && localStorage.BanMsgs === "false") {
                //In case you want a copy in your console.
                //console.log(msg.a);
                return;
            }
        ...
        return {
            ...
            receive: function(msg) {
                ...
                if(msg.a.includes("http")) {
                    var isolateLink = msg.a.substring(msg.a.indexOf("http"));
                    if(isolateLink.includes(" ")) {
                        isolateLink = msg.a.substring(0, isolateLink.indexOf(" "));
                    }
                    msg.a = msg.a.replace(isolateLink, "<a href=\""+isolateLink+"\" target=\"_blank\">"+ isolateLink +"</a>");
                }
                //EMOTES FAK EU HAITCHE
                /////////////////////////
                var imagehtml1 = "<img src=\"";
                var imagehtml2 = "\"></img>";

                if(twitchEmotes === "true") {
                    for(var i = 0; i<emotes.length; i++) {
                        if(msg.a.includes(emotes[i])){ msg.a = msg.a.replace(new RegExp(emotes[i], 'g'), imagehtml1 + sources[i] + imagehtml2); }
                    }
                }
                /////////////////////////
...
// Ban Messages Toggling
(function() {
    function banMsgsOn() {
        if(window.localStorage) {
            var div = document.querySelector("#inclinations");
            div.innerHTML = "Ban Messages:<br>ON / <a id=\"adsoff\" href=\"#\">OFF</a>";
            div.querySelector("#adsoff").addEventListener("click", banMsgsOff);
            localStorage.BanMsgs = true;
        }
    }
    ...
    function banMsgsOff() {
        if(window.localStorage) {
            var div = document.querySelector("#inclinations");
            div.innerHTML = "Ban Messages:<br><a id=\"adson\" href=\"#\">ON</a> / OFF";
            div.querySelector("#adson").addEventListener("click", banMsgsOn);
            localStorage.BanMsgs = false;
        }
    }

    if(window.localStorage) {
        if(localStorage.BanMsgs === undefined || localStorage.BanMsgs === "true")
            banMsgsOn();
        else
            banMsgsOff();
    } else {
    ...
    function pianoOn() {
        if(document.querySelector("#togglepiano") === null) {
            var newDiv = document.createElement("div");
            newDiv.setAttribute("id", "togglepiano");
            newDiv.innerHTML = "Toggle Piano: <br> ON / <a id=\"pianooff\" href=\"#\">OFF </a>";
            document.querySelector("#social").appendChild(newDiv);
            document.querySelector("#pianooff").addEventListener("click", pianoOff);

        } else {
            var div = document.querySelector("#togglepiano");
            div.innerHTML = "Toggle Piano: <br> ON / <a id=\"pianooff\" href=\"#\">OFF </a>";
            div.querySelector("#pianooff").addEventListener("click", pianoOff);
            document.querySelector("#piano").style.display = "";
        }
    }

    function pianoOff() {
        var div = document.querySelector("#togglepiano");
        div.innerHTML = "Toggle Piano: <br><a id=\"pianoon\" href=\"#\"> ON</a> / OFF ";
        div.querySelector("#pianoon").addEventListener("click", pianoOn);
