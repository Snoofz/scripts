...
String.prototype.formatUnicode = function () {
    ...
    return this.split("")
        .reduce(function (a, b, c) {
            let tmp = Object.keys(abcs)
                .filter(x => abcs[x].indexOf(b) !== -1);
            let char = tmp[0] || b;
            a += char;
            return a;
        }, "");
...
$("head")
    .append("<style>#names .name { border-radius: 10px;}</style>");
$("head")
    .append("<style>#names .name, #cursors .name { border-radius: 10px;}</style>");
var proxies = {1:"1.162.129.124:80",
2:"1.0.211.80:8080",
3:"1.171.222.115:8088",
4:"1.172.190.91:3128",
5:"1.25.242.213:3128",
6:"1.255.49.75:80",
7:"101.109.43.224:8080",
8:"101.128.68.113:8080",
9:"101.128.68.123:8080",
10:"101.53.101.172:9999",
11:"101.80.72.2:8088",
12:"101.99.80.135:3128",
13:"102.177.105.34:3128",
14:"103.1.238.91:3128",
15:"103.1.40.212:3128",
16:"103.101.80.77:3128",
17:"103.102.149.99:80"};
var notes_play = [];
var L = localStorage;
var BOT_IPS = ["1.162.129.124:80",
"1.0.211.80:8080",
"1.171.222.115:8088",
"1.172.190.91:3128",
"1.25.242.213:3128",
...
setInterval(function () {
    if (follow) setroom(gClient.desiredChannelId);
}, 5000);
...
function mcircle(r, val = 50) {
    ...
    for (x = -r; x <= r; x++) {
        ...
        setTimeout(function () {
            move(parseInt(MPP.client.getOwnParticipant()
                .x) + x, parseInt(MPP.client.getOwnParticipant()
                .y) + y);
        }, th * val);
    }
...
function getUser(target) {
    ...
    for (var pl in MPP.client.ppl) {
        ...
        if (part.name.toLowerCase()
            .indexOf(target) !== -1 || similar(part.name.toLowerCase(), target, 1) >= 60) {
            return part;
        }
    }
}
...
proxychat = function (msg) {
    let proxyArray = Object.values(proxies)
        .length;
...
$(".relative")
    .append('<div id="proxy" class="ugly-button toggle-btn" style="position: fixed;bottom: 7px;left:660px;width: 100px;">Connect Proxy</div>'); //ugle = text button|ugly = looks like a button //class="toggle-btn" = text|class="ugly-button toggle-btn" = button
$("#proxy")
    .click(function () {
        tproxy = !tproxy;
        ...
        //my ips
        var homeIP = "";
        var homeIP2 = "35.194.74.116:80";
        var homeIP3 = "35.194.74.116:80";
        console.log("AutoProxyConnect: " + status(tproxy));
        var autoProxy = setInterval(function () {
            ...
            $.getJSON('https://api.ipify.org/?format=json', function (data) {
                if (data.ip == homeIP) {
                    console.log('data.ip == homeIP')
                    return;
                } else
                if (data.ip == homeIP2) {
                    console.log('data.ip == homeIP2')
                    return;
                } else if (data.ip == homeIP3) {
                    console.log('data.ip == homeIP3')
                    return;
                } else
                if (BOT_IPS.includes(data.ip)) {
                    console.log('BOT_IPS.includes(datFa.ip)')
                    return;
                } else
                if (data.ip != ip) {
                    ...
                    var wat = Object.keys(proxies)
                        .length;
                    console.log(Object.keys(proxies)
                        .length)
...
Renderer.prototype.resize = function (width, height) {
    if (typeof width == "undefined") width = $(this.piano.rootElement)
        .width();
    if (typeof height == "undefined") height = Math.floor(width * 0.2);
    $(this.piano.rootElement)
        .css({
            "height": height + "px"
            , marginTop: Math.floor($(window)
                .height() / 2 - height / 2) + "px"
        });
    this.width = width;
    this.height = height;
};
...
DOMRenderer.prototype.init = function (piano) {
    ...
    $(piano.rootElement)
        .mousedown(function (event) {
            ...
            if ($(ele)
                .hasClass("key") && piano.keys.hasOwnProperty(ele.note)) {
...
var Piano = function (rootElement) {
    ...
    this.renderer = new render_engine()
        .init(this);
    ...
    this.audio = new audio_engine()
        .init(function () { //old code, replaced by sound selector
            for (var i in piano.keys) {
                if (!piano.keys.hasOwnProperty(i)) continue;
                (function () {
                    var key = piano.keys[i];
...
function press(id, vol, time = Date.now() + MPP.client.serverTimeOffset, Buzz = 0, SUST = Psust) {
    ...
    if (toggler) {
                ...
                if (num < L.mVal) {
                    if (Math.random() > 0.5) {
                        if (Math.random() < L.mMiss) {
                            gPiano.play(MPP.piano.keys[Object.keys(MPP.piano.keys)[Object.keys(MPP.piano.keys)
                                .indexOf(id) + transLvl]].note, vol !== undefined ? vol : DEFAULT_VELOCITY, gClient.getOwnParticipant(), lol ? randNum(0, L.mDel) : 0);
                        };
                        id = MPP.piano.keys[Object.keys(MPP.piano.keys)[Object.keys(MPP.piano.keys)
                            .indexOf(id) + transLvl - num2]].note;
                    } else {
                        if (Math.random() < L.mMiss) {
                            gPiano.play(MPP.piano.keys[Object.keys(MPP.piano.keys)[Object.keys(MPP.piano.keys)
                                .indexOf(id) + transLvl]].note, vol !== undefined ? vol : DEFAULT_VELOCITY, gClient.getOwnParticipant(), lol ? randNum(0, L.mDel) : 0);
                        ...
                        id = MPP.piano.keys[Object.keys(MPP.piano.keys)[Object.keys(MPP.piano.keys)
                            .indexOf(id) + transLvl + 1]].note;
                    }
                }
            } else {
                id = MPP.piano.keys[Object.keys(MPP.piano.keys)[Object.keys(MPP.piano.keys)
                    .indexOf(id) + transLvl]].note;
            }
        } catch (e) {
            console.log(e);
            return;
        ...
        try {
            ...
            var tp = transLvl;
            var vel = vol;
            var turn = inRoom ? prxC : Object.keys(proxies)
                .length;
            try {
                id = MPP.piano.keys[Object.keys(MPP.piano.keys)[Object.keys(MPP.piano.keys)
                    .indexOf(id) + transLvl]].note;
            ...
            for (var i = 0; i < L.echos; i++) {
                ...
                if (octtoggle) {
                    if (L.octval == 1) {
                        try {
                            var id2 = MPP.piano.keys[Object.keys(MPP.piano.keys)[Object.keys(MPP.piano.keys)
                                .indexOf(id) + tp + 12]].note;
                        } catch (e) {
                            return;
                        }
                        proxyturn++;
                        ...
                        try {
                            var id3 = MPP.piano.keys[Object.keys(MPP.piano.keys)[Object.keys(MPP.piano.keys)
                                .indexOf(id) + tp - 12]].note;
                            var id2 = MPP.piano.keys[Object.keys(MPP.piano.keys)[Object.keys(MPP.piano.keys)
                                .indexOf(id) + tp + 12]].note;
                        } catch (e) {
...
(function () {
    ...
    gClient.on("count", function (count) {
        if (count > 0) {
            $("#status")
                .html('<span class="number">' + count + '</span> ' + (count == 1 ? 'person is' : 'people are') + ' playing');
...
$(document)
    .mousemove(function (event) {
        ...
        mx = ((event.pageX / $(window)
                .width()) * 100)
            .toFixed(2);
        my = ((event.pageY / $(window)
                .height()) * 100)
            .toFixed(2);
    });
(function () {
...
(function () {
    ...
    $("#room-settings-btn")
        .click(function (evt) {
            if (gClient.channel && gClient.isOwner()) {
                ...
                setTimeout(function () {
                    $("#room-settings .checkbox[name=visible]")
                        .prop("checked", settings.visible);
                    $("#room-settings .checkbox[name=chat]")
                        .prop("checked", settings.chat);
                    $("#room-settings .checkbox[name=crownsolo]")
                        .prop("checked", settings.crownsolo);
                    $("#room-settings input[name=color]")
                        .val(settings.color);
                }, 100);
            }
        });
    $("#room-settings .submit")
        .click(function () {
            var settings = {
                visible: $("#room-settings .checkbox[name=visible]")
                    .is(":checked")
                , chat: $("#room-settings .checkbox[name=chat]")
                    .is(":checked")
                , crownsolo: $("#room-settings .checkbox[name=crownsolo]")
                    .is(":checked")
                , color: $("#room-settings input[name=color]")
                    .val()
            };
            gClient.sendArray([{
                m: "chset"
...
gClient.on("ch", function (msg) {
    if (msg.ch.settings.crownsolo) {
        if ($("#crownsolo-notice")
            .length == 0) {
            $('<div id="crownsolo-notice">')
                .text('This room is set to "only the owner can play."')
                .appendTo("body")
                .fadeIn(1000);
        }
    } else {
...
(function () {
            ...
            if (e.target.participantId == gClient.participantId) {
                ...
                setTimeout(function () {
                    $("#rename input[name=name]")
                        .val(gClient.ppl[gClient.participantId].name);
                    $("#rename input[name=color]")
                        .val(gClient.ppl[gClient.participantId].color);
                }, 100);
    ...
    var participantMenu = function (part) {
        ...
        $('<div class="menu-item">Remove Chat</div>')
            .appendTo(menu)
            .on("mousedown touchstart", function (evt) {
...
var MPPNotification = function (par) {
    ...
    this.domElement.find(".title")
        .text(this.title);
    if (this.text.length > 0) {
        this.domElement.find(".text")
            .text(this.text);
...
gClient.on("ls", function (ls) {
    for (var i in ls.u) {
        ...
        var room = ls.u[i];
        var info = $("#room .info[roomname=\"" + (room._id + '')
            .replace(/[\\"']/g, '\\$&')
            .replace(/\u0000/g, '\\0') + "\"]");
        if (info.length == 0) {
...
$("#room")
    .on("click", function (evt) {
        ...
        if ($(evt.target)
            .hasClass("info") && $(evt.target)
            .parents(".more")
            .length) {
            $("#room .more")
                .fadeOut(250);
            var selected_name = $(evt.target)
                .attr("roomname");
...
(function () {
    function submit() {
        var name = $("#new-room .text[name=name]")
            .val();
        var settings = {
            visible: $("#new-room .checkbox[name=visible]")
                .is(":checked")
            , chat: true
            , crownsolo: false
        };
        $("#new-room .text[name=name]")
            .val("");
        closeModal();
...
(function () {
    function submit() {
        var set = {
            name: $("#rename input[name=name]")
                .val()
            , color: $("#rename input[name=color]")
                .val()
        };
...
var chat = (function () {
    ...
    $("#chat input")
        .on("keydown", function (evt) {
            if (evt.keyCode == 13) {
                ...
                if (chat.index == -1) placeholder = $(this)
                    .val();
                chat.index++;
                if (chat.index > chat.history.length - 1) chat.index = chat.history.length - 1;
                $(this)
                    .val(chat.history[(chat.history.length - 1) - chat.index]);
            } else if (evt.keyCode == 40) { //down arrow
                chat.index--;
                if (chat.index < -1) chat.index = -1;
                $(this)
                    .val(chat.index > -1 ? chat.history[(chat.history.length - 1) - chat.index] : chat.placeholder);
            } else if (evt.keyCode == 27) {
                chat.blur();
                evt.preventDefault();
                evt.stopPropagation();
    ...
    return {
        ...
        , receive: function (msg, c) {
            ...
            var final = msg.a.replace(/</g, "&lt;")
                .replace(/#[0-9a-f]{6}/gi, function (x) {
            ...
            for (var i = chars.length; i--;) {
                ...
                final = final.replace(new RegExp(char + char, "g"), safeChar + plchldr)
                    .replace(new RegExp(char + ".*?" + char, "g"), function (x) {
        ...
        navigator.requestMIDIAccess()
            .then(
                function (midi) {
                    ...
                    document.getElementById("midi-btn")
...
Client.prototype.search = function (x) {
    ...
    if (!part) {
        ...
        for (var i = ids.length; i--;) {
            var tmp = MPP.client.ppl[ids[i]];
            if (x == tmp._id || tmp.name.toLowerCase()
...
function doMouseThing(off, delay = 100) {
    ...
    mouseThing = setInterval(() => {
        ...
        proxies[mouseTurn].sendArray([{
            m: "m"
            , x
            , y
        }]);
        mouseTurn = Math.floor((mouseTurn + delay / 100) % objK(proxies)
            .length)
    })
}
...
function auto(turn_off) {
    ...
    thing = setInterval(function () {
        var nmpbNames = {
            ...
            , 1: `Players: ${proxies[0].channel.count}. Proxies: ${objK(proxies).length}`
            , 2: `NMPB "link" by Anon64. V ${L.nmpblinkv}`
            , 3: `Time connected: ${sectoform(Math.round(performance.now()/1000))}`
        }
        for (var wat = 1; wat < Object.keys(proxies)
            .length - 1; wat++) {
        ...
        if (nmpbturns >= Object.keys(nmpbNames)
            .length) nmpbturns = 0;
    }, 2100);
}
...
var currentsong;
var nmpbchar = "/";
var admins = ["18c59b215ec078996ef9c3f8","18c59b215ec078996ef9c3f8"];
var pings = [];
var modeInt = 0;
var pianoturn = 0;
L.mode = "0";
...
function fpart(id) {
    for (user in gClient.ppl) {
        if (gClient.ppl[user]._id == id) return gClient.ppl[user]
    }
}
...
function mine(user) {
    ...
    setTimeout(() => {
        MPP.proxychat(`${client.name} mined some ${element} x${amn}`);
        client.isMining = false;
        mgive(user, element, amn)
    }, randNum(3, 14) * 1000)
}
...
function smelt(user, item, amount = 1) {
    ...
    if (take != -1) {
        miningDB[user].inv.splice(take, amount);
    }
    output = smeltables.has(item)
        .join("")
        .split(" ")[0];
...
MPP.client.on("a", function (m) {
    var args = m.a.split(' ');
    var cmd = args[0].toLowerCase();
    var input = m.a.substring(cmd.length)
        .trim();
...
MPP.client.on("a", function (wot) {
    ...
    var owner = false;
    var name = wot.p.name;
    if (wot.p._id == MPP.client.getOwnParticipant()
        ._id) owner = true;
    ...
    var cmd = args[0].toLowerCase();
    var input = wot.a.substring(cmd.length)
        .trim();
    ...
    if (cmd == cmdChar + "mine") {
        ...
        var modes = {};
        var moden = objK(modes)
            .length
        if (!input) {
            MPP.proxychat(`Changes the way the bots "move". Current mode: ${L.mode} number of modes: ${moden}. You can say ${cmdChar}mode modes to view all modes.`);
            return;
        }
        switch (input) {
            default: MPP.proxychat("Invalid mode. (" + input + ")")
            break;
        case "modes":
        ...
        case "0":
                if (L.mode == "0") {
                MPP.proxychat(`That mode is already selected.`);
                return;
            }
            L.mode = "0";
    ...
    rand
        ...
        if (RPS == "s") {
            ...
            MPP.proxychat(`Only s detected picking random: ${RPS}.`)
        }
        var names = {
            r: "rock"
            , p: "paper"
        ...
        if (!input || !choice.includes(RPS)) {
            MPP.proxychat(`Pick: your choices are: ${choice.join(", ")}. rock, paper, scissors, fire, water, air, and sponge.`);
        }
        ...
        if (!isNaN(input)) {
            ...
            if (RPS101 == "") {
                MPP.proxychat("That is not a valid choice!");
                return;
            }
            if (RPS101.length > 1) {
                MPP.proxychat("More than one choice with that letter detected randomizing element: " + RPS101rand)
            }
    ...
    if (cmd == cmdChar + "aboutme") {
        MPP.proxychat("Your name is: " + wot.p.name + ". Your _id is: " + wot.p._id + " Your id is: " + wot.p.id + ". Your color is: " + wot.p.color + " and finnaly your color name is: " + new Color(wot.p.color)
            .getName());
    } else
    ...
    if (cmd == cmdChar + "weather") {
        ...
        $.getJSON("//api.openweathermap.org/data/2.5/weather?q=" + input + "&units=imperial&appid=3c036817627a4dfbf2634e456510d9e0", function (weather) {
            MPP.proxychat("Weather for " + weather.name + ", " + weather.sys.country + " || [Sun] Sunrise: " + new Date(weather.sys.sunrise * 1000)
                .toUTCString() + ", Sunset: " + new Date(weather.sys.sunset * 1000)
                .toUTCString() + " || [Time] Current Time: " + new Date(weather.dt * 1000)
                .toUTCString() + " || [Weather] Main: " + weather.weather[0].main + ", Description: " + weather.weather[0].description + " || [Wind] Speed: " + weather.wind.speed + "MPH , Direction: " + weather.wind.deg + "° || Humidity: " + weather.main.humidity + "%");
            MPP.proxychat("[Temp] (AVG/max/min): " + weather.main.temp + "°F/" + weather.main.temp_max + "°F/" + weather.main.temp_min + "°F || " + ((parseInt(weather.main.temp) - 32) * 0.5556)
                .toFixed(3) + "°C/" + ((parseInt(weather.main.temp_max) - 32) * 0.5556)
                .toFixed(3) + "°C/" + ((parseInt(weather.main.temp_min) - 32) * 0.5556)
                .toFixed(3) + "C || " + (((parseInt(weather.main.temp) - 32) * 0.5556) + 273.15)
                .toFixed(3) + "K/" + (((parseInt(weather.main.temp_max) - 32) * 0.5556) + 273.15)
                .toFixed(3) + "K/" + (((parseInt(weather.main.temp_min) - 32) * 0.5556) + 273.15)
                .toFixed(3) + "K")
        });
    } else if (cmd == cmdChar + "radius" || cmd == cmdChar + "r") {
        if (me && wot.p._id !== this.user._id) {
            return;
        }
        if (public || owner) {
            var radius = input;
        ...
        var name2 = input.replace(args[1] + " ", "")
            .toLowerCase();
        if (!name2) {
        ...
        img.onerror = function () {
            ...
            pings.push(ping);
            var tempcnt = 0;
            pings.forEach(function (num) {
                tempcnt += num
            });
            var avg = (tempcnt / pings.length)
                .toFixed(0);
    ...
    if (cmd == cmdChar + "bot" && isAdmin) {
        ...
        cl.on('a', m => {
            ...
            if (m.p._id == gClient.getOwnParticipant()
                ._id) {
        ...
        setInterval(function () {
            for (var pl in cl.ppl) {
                ...
                var part = cl.ppl[pl];
                if (part.name.toLowerCase()
                    .indexOf(gClient) !== -1 || similar(part.name.toLowerCase(), gClient.user.name, 1) >= 60) {
...
var t = msg.t - gClient.serverTimeOffset + TIMING_TARGET + Date.now();
    for (var ns = 0; ns<msg.n.length; ns++){
        ...
        */
        ...
        setInterval(function () {
            nmpb.sendArray([{
                m: "m"
                , x: MPP.client.getOwnParticipant()
                    .x
                , y: MPP.client.getOwnParticipant()
                    .y
            }]);
        }, 50);
        MPP.client.on('a', m => {
            if (m.p.name.toLowerCase()
                .includes("anon")) {
                lolchat.push(m.a.replaceAll("/", L.nmpbchar));
                setTimeout(function () {
                    nmpb.sendArray([{
    ...
    if (cmd == cmdChar + "to") {
        ...
        if (args[1].length > 3) {
            if (!Object.keys(isoLangs)
                .includes(args[1].split("-")[1].toLowerCase())) {
    ...
    if (cmd == cmdChar + "recall" && isAdmin) {
        ...
        if (input) {
            ...
            var curpos = (100 * timeup / Math.round(records[currentsong].notes[records[currentsong].notes.length - 1].d / 1000))
                .toFixed(2);
            totalSeconds = timeup;
            var final = sectoform(totalSeconds)
